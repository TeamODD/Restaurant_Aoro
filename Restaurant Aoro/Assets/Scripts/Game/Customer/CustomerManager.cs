...
public class CustomerManager : MonoBehaviour
{
    ...
    public ResultType resultTypeOnLastServe;   // ������
    ...
    private void Prewarm(GameObject prefabAsset)
    {
        ...
        if (visualRoot == null)
        {
            Debug.LogError("[CustomerManager] Prewarm: visualRoot�� null (Inspector�� VisualRoot ���� �ʿ�)");
            return;
        }
        ...
        if (found == null)
        {
            ...
            sb.AppendLine($"[CustomerManager] VisualRoot ���� ������Ʈ ��� (�� {all.Length}��):");
            foreach (var t in all)
                sb.AppendLine($"- '{t.name}'");

            Debug.LogError($"[CustomerManager] VisualRoot���� '{prefabAsset.name}'�� �� ã��.\n{sb}");
            return;
        }
    ...
    private void SwitchVisual(GameObject prefabAsset)
    {
        ...
        if (!_spawned.TryGetValue(prefabAsset, out var go) || go == null)
        {
            ...
            {
                Debug.LogError($"[CustomerManager] SwitchVisual ����: {prefabAsset.name}");
                return;
            }
        }
    ...
    private ResultType EvaluateResult(Item served, Customer cust)
    {
        ...
        {
            score += FAVOR_TASTE;
            Debug.Log($"[{cust.CustomerID}] �����ϴ� ��({served.Foodtaste}) �� +{FAVOR_TASTE}");
        }
        else if (cust.dislikedTastes != null && cust.dislikedTastes.Contains(served.Foodtaste))
        {
            score += DISLIKE_TASTE;
            Debug.Log($"[{cust.CustomerID}] �Ⱦ��ϴ� ��({served.Foodtaste}) �� {DISLIKE_TASTE}");
        }

        if (cust.favoriteFoods != null && cust.favoriteFoods.Contains(served.ItemMainCategory))
        {
            score += FAVOR_CATEGORY;
            Debug.Log($"[{cust.CustomerID}] �����ϴ� ����({served.ItemMainCategory}) �� +{FAVOR_CATEGORY}");
        }
        else if (cust.dislikedFoods != null && cust.dislikedFoods.Contains(served.ItemMainCategory))
        {
            score += DISLIKE_CATEGORY;
            Debug.Log($"[{cust.CustomerID}] �Ⱦ��ϴ� ����({served.ItemMainCategory}) �� {DISLIKE_CATEGORY}");
        }

        if (score >= 4)
        {
            Debug.Log($"[{cust.CustomerID}] ���: Perfect ({score})");
            return ResultType.Perfect;
        }
        else if (score >= 2)
        {
            Debug.Log($"[{cust.CustomerID}] ���: Excellent ({score})");
            return ResultType.Excellent;
        }
        else if (score >= 0)
        {
            Debug.Log($"[{cust.CustomerID}] ���: Success ({score})");
            return ResultType.Success;
        }
        else
        {
            Debug.Log($"[{cust.CustomerID}] ���: Fail ({score})");
            return ResultType.Fail;
        }
    }
